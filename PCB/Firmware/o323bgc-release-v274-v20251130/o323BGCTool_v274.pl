...
my $VersionStr = '16. Nov. 2025 v2.74-6';

#CAREFULL: in Data Display version is not double checked!
my $SupportedBGCLayoutVersion = '274'; #supported layout version
my $SupportedBGCVersion = '274'; #minimally required controller version, set to zero if no check required

my @FirmwareVersionList =  ( 'v2.74','v2.73e','v0.96' ); #versions available in the flash tab selector
my @FirmwareVReplaceList = ( 'v274_nt','v273e_nt','v096_nt' ); #versions available in the flash tab selector
...
my @STorM32BoardList = (
{
  ...
  name => 'STorM32 v6.2',             hexfile => 'o323bgc_V_storm32bgc_v620_h7a3rg',
},{
  name => 'STorM32 v6.3',             hexfile => 'o323bgc_V_storm32bgc_v630_h7a3rg',
},{
  name => 'NT Imu Module v4.x',  hexfile => 'o323bgc_ntimu_V_module_v4x_g431kb',
  versions =>  [ 'v0.56e' ],
  vreplaces => [ 'v056e' ],
  boards =>    [ 'v4.x G431KB', ],
},{
  ...
  name => 'NT Motor Module v4.x',     hexfile => 'o323bgc_ntmotor_V_module_v4x_g431kb',
  versions =>  [ 'v0.55e' ],
  vreplaces => [ 'v055e' ],
  boards =>    [ 'v4.xEC G431KB'],
},{
  ...
  name => 'NT Logger Module v2.x',    hexfile => 'o323bgc_ntlogger_V_module_v2x_g431kb',
  versions =>  [ 'v0.56e' ],
  vreplaces => [ 'v056e' ],
  boards =>    [ 'v2.x G431KB' ],
},{
...
NT modules: The firmwares for the NT modules have been modified, please upgrade.
...
my @SetupTabList = (
  'dashboard', 'pid', 'pan', 'rcinputs', 'functions', 'scripts', 'setup',
  'gimbalconfig', 'expert', 'interfaces', 'servo', 'gimbalmodel', 'gimbalmechanics', 'filter' );
my $MaxSetupTabs = scalar(@SetupTabList); #this is the number of setup tabs

my $SCRIPTSIZE = 240; #128;
...
my $CMD_g_PARAMETER_ZAHL = 226; #number of values transmitted with a 'g' get data command
my $CMD_g_PARAMETER_LEN = 303; #length of bytes transmitted with a 'g' get data command

# one longer than SETUP_PARAMETER_ZAHL, allows to determine size from consequative value
my @PosInBuf = (
0,2,4,6,7,9,11,13,14,16,18,20,21,22,23,24,26,27,28,30,31,32,34,35,37,39,41,43,44,45,
47,49,51,53,55,57,59,61,63,65,66,67,68,69,71,72,74,75,77,78,79,81,82,83,84,85,86,87,88,90,
92,94,96,98,100,102,103,104,105,106,107,108,109,110,111,112,113,115,116,117,118,119,120,121,122,123,125,126,127,129,
130,131,133,134,136,137,139,140,141,142,144,146,148,149,150,151,153,155,157,159,160,161,162,164,166,168,170,171,172,173,
175,177,179,181,182,183,184,185,186,187,189,190,192,194,196,198,199,200,201,202,203,204,206,207,208,209,210,211,212,213,
214,215,216,217,218,219,220,221,222,223,224,225,226,227,228,229,230,231,232,233,234,235,236,237,238,239,240,242,244,246,
248,250,252,253,254,255,256,258,259,261,263,264,265,266,267,269,271,273,274,275,276,277,278,279,280,281,282,283,284,285,
286,287,288,289,290,291,292,293,294,295,296,297,298,299,300,301,303,
);
...
##--- PID tab --------------------
  ...
  pos => [1,1],
#},{
#  name => 'Gyro Dead Band', foc => 1, # hide for foc, show for default
#  type => 'UINT8', len => 0, ppos => 0, min => 0, max => 15, default => 0, steps => 1,
#  adr => 155,
},{
  name => 'Imu2 FeedForward LPF', foc => 1, # hide for foc, show for default
  type => 'LIST', len => 0, ppos => 0, min => 0, max => 6, default => 1, steps => 1,
  adr => 72,
  choices => [ 'off', '1.5 ms', '4 ms', '10 ms', '22 ms', '46 ms', '94 ms' ],
},{
  name => 'Voltage Correction', foc => 1, # hide for foc, show for default
  ...
  pos => [1,1],
#},{
#  name => 'Foc Gyro LPF Type', foc => 2, # show for foc, hide for default
#  type => 'LIST', len => 0, ppos => 0, min => 0, max => 2, default => 0, steps => 1,
#  adr => 39,
#  choices => [ 'LPF1', 'LPF1 bi', 'LPF2' ],
#},{
#  name => 'Foc Gyro Dead Band', foc => 2, # show for foc, hide for default
#  type => 'UINT8', len => 0, ppos => 0, min => 0, max => 15, default => 0, steps => 1,
#  adr => 40,
...
##--- RC INPUTS tab --------------------
  ...
  name => 'Rc Pitch Speed Limit', comment => '(0 = max)',
  type => 'UINT', len => 0, ppos => 1, min => 0, max => 3600, default => 400, steps => 5,
  adr => 107,
  unit => '°/s',
  calc => 'logI', # 0..25: steps = 1 = 0.1°/s, 26..220: steps = 5 = 0.5°/s, 221..246: steps = 100 = 10°/s
},{
  name => 'Rc Pitch Accel Limit', comment => '(0 = off)',
  type => 'UINT', len => 0, ppos => 3, min => 0, max => 1000, default => 300, steps => 10,
  adr => 108,
},{
  name => 'Rc Pitch Expo', comment => '(0 = off)',
  type => 'UINT8', len => 0, ppos => 0, min => 0, max => 100, default => 0, steps => 1,
  adr => 151,
  unit=> '%',
  ...
  name => 'Rc Roll Speed Limit', comment => '(0 = max)',
  type => 'UINT', len => 0, ppos => 1, min => 0, max => 3600, default => 400, steps => 1,
  adr => 114,
  unit => '°/s',
  calc => 'logI', # 0..25: steps = 1 = 0.1°/s, 26..220: steps = 5 = 0.5°/s, 221..246: steps = 100 = 10°/s
},{
  ...
  name => 'Rc Roll Expo', comment => '(0 = off)',
  type => 'UINT8', len => 0, ppos => 0, min => 0, max => 100, default => 0, steps => 1,
  adr => 152,
  unit=> '%',
  ...
  name => 'Rc Yaw Speed Limit', comment => '(0 = max)',
  type => 'UINT', len => 0, ppos => 1, min => 0, max => 3600, default => 400, steps => 1,
  adr => 121,
  unit => '°/s',
  calc => 'logI', # 0..25: steps = 1 = 0.1°/s, 26..220: steps = 5 = 0.5°/s, 221..246: steps = 100 = 10°/s
  ...
  name => 'Rc Yaw Expo', comment => '(0 = off)',
  type => 'UINT8', len => 0, ppos => 0, min => 0, max => 100, default => 0, steps => 1,
  adr => 153,
  unit=> '%',
...
##--- FUNCTIONS tab --------------------
  ...
  name => 'Re-center Camera Mode',
  type => 'LIST', len => 0, ppos => 0, min => 0, max => 1, default => 0, steps => 1,
  adr => 156,
  choices => [ 'unwind', 'shortest path' ],
},{
  name => 'Retract',
  type => 'LIST', len => 0, ppos => 0, min => 0, max => $FunctionInputMax, default => 0, steps => 1,
  adr => 157,
...
#  presets => [ 'Clear' => 0 ],
  ...
  adr => 174,
  choices => [ 'default', 'uart', 'uart2', 'uartX', 'mavlink tunnel ch3' ],
...
#           'NTLogger working'],
  ...
  type => 'LIST', len => 0, ppos => 0, min => 0, max => 10, default => 0, steps => 1,
  adr => 77,
  choices => [ 'off', 'sum ppm auto', 'sbus', 'sbus inverted', 'sbus fast',
...
##--- EXPERT tab  --------------------
  ...
  name => 'Flip Gimbal Control', foc => 3, # show but disable for foc, show and enable else
  type => 'LIST', len => 0, ppos => 0, min => 0, max => 4, default => 0, steps => 1,
  adr => 73,
  choices => [ 'off', 'Aux-0', 'Aux-1', 'Aux-2', 'But' ],
...
##--- GIMBAL MECHANICS tab --------------------
},{
  name => 'Pitch Motor Limit Min', comment => '(0 = off)', foc => 4, # show and enable for foc, show but disable else
  type => 'UINT8', len => 3, ppos => 0, min => 0, max => 180, default => 0, steps => 1,
  adr => 190,
  pre => '-',
  unit => '°',
  page => 'gimbalmechanics',
  pos => [1,1],
},{
  name => 'Pitch Motor Limit Max', foc => 4, # show and enable for foc, show but disable else
  type => 'UINT8', len => 3, ppos => 0, min => 0, max => 180, default => 0, steps => 1,
  adr => 191,
  pre => '+',
  unit => '°',
},{
  name => 'Roll Motor Limit', comment => '(0 = off)', foc => 4, # show and enable for foc, show but disable else
  type => 'UINT8', len => 3, ppos => 0, min => 0, max => 180, default => 0, steps => 1,
  adr => 192,
  pre => '+-',
  unit => '°',
  ...
  name => 'Yaw Motor Limit', comment => '(0=off, 180=inf)', ##foc => 4, # show and enable for foc, show but disable else
  type => 'UINT8', len => 3, ppos => 0, min => 0, max => 180, default => 0, steps => 1,
  adr => 193,
  pre => '+-',
  unit => '°',
  ...
  name => 'Setpoint Speed Limit', comment => '(0 = max)', foc => 4, # show and enable for foc, show but disable else
  type => 'UINT', len => 3, ppos => 0, min => 0, max => 500, default => 0, steps => 25,
  adr => 195, #195: __RangeSpeedLimit, UINT16
  unit => '°/s',
  #multiplier => 10.0,
  pos => [1,4],
},{
  name => 'Setpoint Accel Limit', comment => '(0 = off)', foc => 4, # show and enable for foc, show but disable else
  type => 'UINT', len => 3, ppos => 0, min => 0, max => 1000, default => 0, steps => 50,
  adr => 194, #194: __RangeAccLimit, UINT16
  unit => '°/s/s',
  pos => [2,4],

#},{
#  name => 'Fastlevel Speed Limit', comment => '(0 = max)', foc => 4, # show and enable for foc, show but disable else
#  type => 'UINT8', len => 3, ppos => -1, min => 0, max => 25, default => 0, steps => 1,
#  adr => 198,
#  unit => '°/s',
#  #multiplier => 10.0,
#  pos => [1,5],
#},{
#  name => 'Fastlevel Accel Limit', comment => '(0 = off)', foc => 4, # show and enable for foc, show but disable else
#  type => 'UINT8', len => 3, ppos => 0, min => 0, max => 250, default => 0, steps => 1,
#  adr => 197,
#  unit => '°/s/s',
#  pos => [2,5],
...
##--- FILTER tab --------------------
  ...
  type => 'LIST', len => 0, ppos => 0, min => 0, max => 2, default => 0, steps => 1,
  adr => 209,
  choices => [ 'off', 'gyro1 all', 'imu1 all' ],
  page => 'filter',
  column => 1,
},{
  name => 'Filter1 Center Frequency',
  type => 'UINT8', len => 0, ppos => 0, min => 25, max => 250, default => 100, steps => 1,
  adr => 210,
  unit => 'Hz',
},{
  ...
  type => 'LIST', len => 0, ppos => 0, min => 0, max => 7, default => 2, steps => 1,
  adr => 212,
  choices => [ '-6 dB', '-12 dB', '-20 dB', '-25 dB', '-30 dB', '-35 dB', '-40 dB', 'inf' ],
...
# adds:       size, default, steps, pre, unit, expert, hidden
...
sub SetOptionList{
  ...
  foreach my $Option (@OptionList){
    ...
    if( not defined $Option->{pre} ){ $Option->{pre} = ''; }
    if( not defined $Option->{unit} ){ $Option->{unit} = ''; }
    #if( not defined $Option->{multiplier} ){ $Option->{multiplier} = 1.0; }
    if( not defined $Option->{calc} ){ $Option->{calc} = ''; }
...
my $SystemBootloaderDfu = 'System Bootloader @ USB DFU';
...
my @Storm32ProgrammerListV1X = ( $StLink, $SystemBootloader );
my @Storm32ProgrammerListV3X = ( $Storm32UpgradeViaUSB, $StLink, $SystemBootloader );
my @Storm32ProgrammerListV56X = ( $Storm32UpgradeViaUSB, $StLink, $SystemBootloaderDfu );

my @NtProgrammerList = ( $NtUpgradeViaUSB, $NtFlashViaUSB, $StLink, $SystemBootloader );

my $STorM32Board = '';
my $FirmwareVersion = '';
my $Storm32ProgrammerV1X = $Storm32ProgrammerListV1X[0];
my $Storm32ProgrammerV3X = $Storm32ProgrammerListV3X[0];
my $Storm32ProgrammerV56X = $Storm32ProgrammerListV56X[0];
...
if( defined $IniFile ){
  ...
  if( defined $IniFile->val('FLASH','Stm32ProgrammerV1X') ){ $Storm32ProgrammerV1X= $IniFile->val('FLASH','Stm32ProgrammerV1X'); }
  if( defined $IniFile->val('FLASH','Stm32ProgrammerV3X') ){ $Storm32ProgrammerV3X= $IniFile->val('FLASH','Stm32ProgrammerV3X'); }
  if( defined $IniFile->val('FLASH','Stm32ProgrammerV56X') ){ $Storm32ProgrammerV56X= $IniFile->val('FLASH','Stm32ProgrammerV56X'); }
...
if( not grep{$_ eq $Storm32ProgrammerV1X} @Storm32ProgrammerListV1X ){ $Storm32ProgrammerV1X= $Storm32ProgrammerListV1X[0]; }
if( not grep{$_ eq $Storm32ProgrammerV3X} @Storm32ProgrammerListV3X ){ $Storm32ProgrammerV3X= $Storm32ProgrammerListV3X[0]; }
if( not grep{$_ eq $Storm32ProgrammerV56X} @Storm32ProgrammerListV3X ){ $Storm32ProgrammerV56X= $Storm32ProgrammerListV56X[0]; }
...
#    '>Store to EEPROM', 'm_StoreSettings',
  ...
  'Tools' => '',
    ...
    ">NT Logger RTC Tool...", 't_RTCConfigTool',
    ">-", 0,
  ...
  'Experts Only' => '',
    ...
    ">Gimbal Mechanics Tab...", 't_GimbalMechanicsTool',
...
#$f_Tab{gimbalmechanics} = $w_Main->w_Tab->InsertItem(-text=> '     Gimbal Mechanics');
...
$f_Tab{gimbalmechanics} = Win32::GUI::DialogBox->new( -name=> 'm_GimbalMechanicsWindow', -parent => $w_Main,
  -text=>'o323BGC Gimbal Mechanics Tab',
  -size=> [10+4*$OPTIONSWIDTH_X,360-2], -pos=> [$DialogXPos+$xsize-380-180,$DialogYPos+50], -helpbox => 0,
);
$f_Tab{gimbalmechanics}->Hide();

sub t_GimbalMechanicsTool_Click{
  my ($x,$y)=($w_Main->GetWindowRect())[0..1];
  $f_Tab{gimbalmechanics}->Move($x+$ColNumber*$OPTIONSWIDTH_X+40-15+20-385-180+1-180,$y+50-12);
  $f_Tab{gimbalmechanics}->Show(); 0;
}
sub m_GimbalMechanicsWindow_Terminate{ $f_Tab{gimbalmechanics}->Hide(); 0; }
...
push(@f_TabWindows, {'tab'=>$f_Tab{gimbalmechanics}, 'index'=>GetTabIndex('gimbalmechanics')} );
...
$xpos= 655;
$f_Tab{calibrateacc}->AddLabel( -name=> 'caac_Sensor_label', -font=> $StdWinFont,
  -text=> 'Sensor', -pos=> [$xpos,$ypos-3 + 1*13],
);
$f_Tab{calibrateacc}->AddRadioButton( -name=> 'caac_SensorAcc', -font=> $StdWinFont,
  -text=> 'Acc', -pos=> [$xpos,$ypos-3 + 2.5*13], -width=> 100,
  -checked=>1,
  -onClick => sub{
...
$f_Tab{calibrateacc}->AddRadioButton( -name=> 'caac_SensorGyro', -font=> $StdWinFont,
  -text=> 'Gyro', -pos=> [$xpos,$ypos-3 + 4.0*13], -width=> 100,
  -onClick => sub{
      #CalibrateTabClearCalibration();
...
$f_Tab{calibrateacc}->AddRadioButton( -name=> 'caac_SensorAccGyro', -font=> $StdWinFont,
  -text=> 'Acc+Gyro', -pos=> [$xpos,$ypos-3 + 5.5*13], -width=> 100,
  -onClick => sub{
      #CalibrateTabClearCalibration();
      $f_Tab{calibrateacc}->caac_Run1PointCalibration->Enable(1);
      $f_Tab{calibrateacc}->caac_Run6PointCalibration->Enable(1);
...
Store the selected calibration data to the EEPROM, otherwise they won't be active nor permanent.
...
#STorM32 SystemBootLoader at Uart1 for V1x
...
#STorM32 SystemBootLoader at Uart1 for V3x
...
#STorM32 SystemBootLoader at Usb for V5x
$f_Tab{flash}->AddLabel( -name=> 'flash_ProgrammerUsage_Storm32v56x_SysBootLoad_label', -font=> $StdWinFont,
  -pos=> [$xpos+430+35,$ypos+30], -multiline => 1, -height=>6*13+50, -width=>300,
  -text=> 'Usage:
1. Connect STorM32 board via usb
2. Select port
...
#STorM32 Upgrade via USB for V3x, V4X, V5x
...
my $BOARDTYPE_IS_NTMODULE = 0;
my $BOARDTYPE_IS_STORM32_V1X = 1;
my $BOARDTYPE_IS_STORM32_V3X = 3; #this allows Upgrade via USB (COMxx), and @ UART1
my $BOARDTYPE_IS_STORM32_V56X = 5; #this allows Upgrade via USB (COMxx), and native USB (usb1)
...
sub GetBoardTypeAndName{
  ...
  my $boardtype = $BOARDTYPE_IS_STORM32_V1X;
  if( $boardname =~ /^STorM32 v3./ ){ $boardtype = $BOARDTYPE_IS_STORM32_V3X; }
  if( $boardname =~ /^STorM32 v4./ ){ $boardtype = $BOARDTYPE_IS_STORM32_V3X; }
  if( $boardname =~ /^STorM32 v5./ ){ $boardtype = $BOARDTYPE_IS_STORM32_V56X; }
  if( $boardname =~ /^STorM32 v6./ ){ $boardtype = $BOARDTYPE_IS_STORM32_V56X; }
...
sub SetFirmwareHexFile{
  ...
  if( $boardtype == $BOARDTYPE_IS_NTMODULE ){ #this is a NT module
    ...
    #this is a v1.x, v3.x, v4.x or v5.x board
...
sub SetBoard{
  ...
  if( $boardtype != $lastboardtype ){ #type has been changed
    ...
    }elsif( $boardtype == $BOARDTYPE_IS_STORM32_V56X ){ #this is a v5.x/v6.x board, same here as v3
      $f_Tab{flash}->flash_Programmer->Add( @Storm32ProgrammerListV56X );
      $f_Tab{flash}->flash_Programmer->SelectString( $Storm32ProgrammerV56X );
    }else{
      #good for v1.x boards
      $f_Tab{flash}->flash_Programmer->Add( @Storm32ProgrammerListV1X );
      $f_Tab{flash}->flash_Programmer->SelectString( $Storm32ProgrammerV1X );
      # WE NEED TO CHECK IF ENTRY WAS VALID
    }
    if( $f_Tab{flash}->flash_Programmer->GetCurSel() < 0 ){ #no valid entry found
      $Storm32ProgrammerV1X = $SystemBootloader; #$f_Tab{flash}->flash_Programmer->GetString(0);
      $f_Tab{flash}->flash_Programmer->SelectString( $Storm32ProgrammerV1X );
    }
  }
...
sub SetProgrammer{
  ...
  }elsif( $boardtype == $BOARDTYPE_IS_STORM32_V56X ){ #this is a v5.x/v6.x board
    $Storm32ProgrammerV56X = $pstr;
  }else{ #this is a v1.x board
    $Storm32ProgrammerV1X = $pstr;
  }
  ...
  $f_Tab{flash}->flash_ProgrammerUsage_Storm32v56x_SysBootLoad_label->Hide();
  ...
  # system bootloader, all boards
  if( $pstr eq $SystemBootloader ){
    if( $boardtype == $BOARDTYPE_IS_NTMODULE ){ #this is a NT module
      $f_Tab{flash}->flash_ProgrammerComPort_label->Show(); #COM is needed for NT modules
      $f_Tab{flash}->flash_ProgrammerComPort->Show();
      $f_Tab{flash}->flash_ProgrammerUsage_NtSysBootLoad_label->Show();
    }elsif( $boardtype == $BOARDTYPE_IS_STORM32_V3X ){ #this is a v3.x or v4.x board
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v3x_SysBootLoad_label->Show();
    }else{ #this ia a v1.x board
      $f_Tab{flash}->flash_ProgrammerComPort_label->Show(); #COM is needed for v1.x boards
      $f_Tab{flash}->flash_ProgrammerComPort->Show();
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v1x_SysBootLoad_label->Show();
    }

  }elsif( ($boardtype == $BOARDTYPE_IS_STORM32_V56X) and ($pstr eq $SystemBootloaderDfu) ){
      $f_Tab{flash}->flash_ProgrammerUsage_Storm32v56x_SysBootLoad_label->Show();

  }elsif( ($boardtype == $BOARDTYPE_IS_STORM32_V3X or $boardtype == $BOARDTYPE_IS_STORM32_V56X)
           and ($pstr eq $Storm32UpgradeViaUSB) ){
    $f_Tab{flash}->flash_ProgrammerUsage_Storm32v3x_UpgradeViaUsb_label->Show();
    ...
    $f_Tab{flash}->flash_ProgrammerComPort->Show();
    $f_Tab{flash}->flash_ProgrammerUsage_NtUpgradeViaSysBootLoad_label->Show();
    ...
    $f_Tab{flash}->flash_ProgrammerUsage_NtUpgradeViaStorm32Usb_label->Show();

  }elsif( ($boardtype == $BOARDTYPE_IS_NTMODULE) and ($pstr eq $NtFlashViaUSB) ){
    $f_Tab{flash}->flash_ProgrammerUsage_NtFlashViaStorm32Usb_label->Show();
...
sub GetLogIValueFromPos{
  my $pos = shift;
  if( $pos <= 25 ){ # pos = 0..25 -> 0.0..2.5
    return $pos;
  }
  if( $pos <= 220){ # pos = 26..220 -> 3.0..100.0
    return ($pos - 20) * 5;
  }
  # pos = 221..246 -> 110.0..360.0
  return ($pos - 210) * 100;
}

sub GetLogIPosFromValue{
  my $value = shift;
  if( $value <= 25 ){
    return $value; # pos = 0..25
  }
  if( $value <= 1000 ){
    return $value/5 + 20; # pos = 26..220
  }
  return $value/100 + 210; # pos = 221..246
}
...
sub ConvertOptionToStr{
  ...
  if( defined $Option->{multiplier} ) { $value = $value * $Option->{multiplier}; }
  return sprintf( "%.".$ppos."f", $value );
}

sub ConvertOptionToStrWUnit{
  my $Option = shift; my $value = shift;
  if ($value == 0) { return ConvertOptionToStr($Option, $value)." ".$Option->{unit}; }
  return $Option->{pre}.ConvertOptionToStr($Option, $value)." ".$Option->{unit};
}
...
sub onScrollSetTextfield{
  ...
  switch( $Option->{type} ){
    ...
    case ['UINT','INT','UINT8','INT8', ] {
      my $value;
      if( $Option->{calc} eq 'logI' ){
        my $pos = $Option->{setfield}->GetPos();
#        if( $pos <= 25 ){ # pos = 0..25 -> 0.0..2.5
#          $value = $pos;
#        }elsif( $pos <= 220){ # pos = 26..220 -> 3.0..100.0
#          $value = ($pos - 20) * 5;
#        }else{ # pos = 221..246 -> 110.0..360.0
#          $value = ($pos - 210) * 100;
#        }
        $value = GetLogIValueFromPos($pos);
      }else{
...
sub ConvertOptionBitmaskFromValue{
  ...
  if( $value == 0 ){
    ...
    if( $maxvalue == 1 ){ $Option->{textfield}->Text( lc($bitname) ); }else{ $Option->{textfield}->Text( 'all' ); }
    return;
  }elsif( $bitsset == 1 ){
...
sub SetOptionField{
  ...
  switch( $Option->{type} ){
    ...
    case ['UINT','UINT8'] {
      if( $Option->{calc} eq 'logI' ){
#        if( $value <= 25 ){
#          $Option->{setfield}->SetPos( $value ); # pos = 0..25
#        }elsif( $value <= 1000 ){
#          $Option->{setfield}->SetPos( $value/5 + 20 ); # pos = 26..220
#        }else{
#          $Option->{setfield}->SetPos( $value/100 + 210 ); # pos = 221..246
#        }
        $Option->{setfield}->SetPos( GetLogIPosFromValue($value) );
      }else{
...
sub flash_HexFile_button_Click{
  my $file= Win32::GUI::GetOpenFileName( -owner=> $w_Main,
    ...
    -filter=> ['firmware files'=>'*.hex;*.bin;*.elf', 'All files' => '*.*'],
    -pathmustexist=> 1,
    -filemustexist=> 1,
  );
  if( $file ){
...
my $NTBUS_IMU_CONFIG_EXTERNALCLK    =  6; #0x0200
...
my $NTBUS_MOTOR_CONFIG_MA732        = 13; #0x0004
...
sub NtScanNsResultToStr{
  ...
  if( length($test) == 0 ){
    ...
    if( ($id eq '01') or ($id eq '02') or ($id eq '0C') ){
      ...
      if( CheckStatus($config,$NTBUS_IMU_CONFIG_EXTERNALCLK) ){ $str .= 'ECLK '; $res{'filter'} .= 'EXTERNALCLK ';  }
    }
...
#      }
      ...
      elsif( $enctype == 4 ){ $str .= 'MA732 ';    $res{'encoder'} = 'MA732';    }
    }
  }
  return ($str,%res);
}
...
##pre-checks
  ...
  if( (($boardtype == $BOARDTYPE_IS_NTMODULE) and (($pstr eq $NtUpgradeViaUSB) or ($pstr eq $NtFlashViaUSB))) ){
    #if( index($file, "g431") != -1 ){
    if( $file =~ m/g431/ ){
      my $res = $w_Main->MessageBox(
        "You are about to upgrade a NT module\n".
        "with a STM32G4 processor.\n\n".
        "It will need to be power-cycled after flashing!"
        ,
        'Upgrade Firmware' , 0x0030 + 1 ); #0x0010 + 1 ); # MB_ICONHAND +  MB_OKCANCEL
      if( $res != 1 ){ TextOut( "\nFlash firmware... CANCELED!\n" ); return 1; }
    }
  ...
  if( ($boardtype == $BOARDTYPE_IS_STORM32_V3X or $boardtype == $BOARDTYPE_IS_STORM32_V56X)
       and ($pstr eq $Storm32UpgradeViaUSB) ){
    #upgrading v3.x, v4.x, v5.x per usb needs its special handling, since it should work independent on version/layout!
    if( not ConnectionIsValid() ){
      if( not OpenPort() ){
        ClosePort(); TextOut( "\nFlash firmware... ABORTED!\nConnection to STorM32 board failed!\n" ); return 1;
      }
      #check connection
      my ($Version,$Name,$Board,$Layout,$Capabilities,$VerNr) = GetControllerVersion();
...
##flash using SystemBootloader, in one or the other way
  ...
  if( (($boardtype == $BOARDTYPE_IS_STORM32_V1X) and ($pstr eq $SystemBootloader)) or
      (($boardtype == $BOARDTYPE_IS_NTMODULE) and ($pstr eq $NtUpgradeViaSystemBootloader)) ){
    #these use the [Com Port], different from [Port]
  ...
  }elsif( (($boardtype == $BOARDTYPE_IS_STORM32_V3X) and ($pstr eq $SystemBootloader)) or
          (($boardtype == $BOARDTYPE_IS_STORM32_V56X) and ($pstr eq $SystemBootloaderDfu)) ){
    #these use the normal Port
    # but we do not need the connection via the normal Port later
    # so we should  disconnect here
    SetDoFirstReadOut(0); #we do not want to read back the parameteres
    DisconnectFromBoard(0); #this would happen anyway during a flash
    if( $boardtype == $BOARDTYPE_IS_STORM32_V56X ){
      TextOut( "\nuse System Bootloader @ USB1 = STorM32 usb port" );
    }else{
      ...
      TextOut( "\nuse System Bootloader @ UART1 via STorM32 usb port" );
            ...
            and ($pstr eq $Storm32UpgradeViaUSB)) or
    ...
    # not needed for v5.x, where it always is usb1
    $StmFlashLoaderPort = ExtractCom($Port);
  ...
  if( ($boardtype == $BOARDTYPE_IS_NTMODULE) and ($pstr eq $NtUpgradeViaUSB) ){
    #we need to send xQTcNTFLASH+id
    #first scan for nt module
    my $module = '';
    my $moduleindex = $f_Tab{flash}->flash_ProgrammerModuleId->GetCurSel();
    if( $moduleindex >= 0 ){ $module = $f_Tab{flash}->flash_ProgrammerModuleId->GetString($moduleindex); }
...
if(0){ ###XXXX
    ...
    #now send xQTFLASH+id to set module into bootloader mode
    ...
    #send flash stx to set NT module into bootloader mode
    ...
    if( (defined $p_ntbus) and ($p_ntbus) ){
      ...
      $p_ntbus->close;
           ...
           and ($pstr eq $Storm32UpgradeViaUSB) ){
    #we need to send xQScFLASH to set STorM32 board into bootloader mode
    if( $boardtype == $BOARDTYPE_IS_STORM32_V56X ){
      TextOut( "\nset STorM32 v5.x/v6.x board into bootloader mode" );
    }else{
  ...
  my $s = '';
  #prepare everything for STM32CubeProgrammer to run, and run it
...
###  $s.= ' -vb 3 -log "bin\ST\log.txt"';
  if( $f_Tab{flash}->flash_FullErase_check->GetCheck() ){ # full erase enabled?
    TextOut( "\ndo full chip erase" );
  ...
  $s .= ' -w "'.$file.'"';
  if( $f_Tab{flash}->flash_Verify_check->GetCheck() ){ # verify enabled?
    TextOut( "\ndo verify" );
    $s .= ' -v';
  }
  $s .= ' -g'; # works for F1, but DOES NOT SEEM TO WORK for G4 ?!?! seems to be true
  #$s .= ' --start 0x08000000';
  $s .= "\n";

  if( $file =~ m/_g4/ or $file =~ m/-g4/ ){
    $s .= '@echo.'."\n";
    #'\033[91m'+txt+'\033[0m') # light Red
    if( $boardtype == $BOARDTYPE_IS_NTMODULE ){
      $s .= '@echo '."\033[91m".'NT Module is using G4, needs to be power cycled !'."\033[0m"."\n";
    } else {
      $s .= '@echo '."\033[91m".'STorM32 board is using G4, needs to be power cycled !'."\033[0m"."\n";
    }
  }
  if( $file =~ m/_h7/ or $file =~ m/-h7/ ){
    $s .= '@echo.'."\n";
    #'\033[91m'+txt+'\033[0m') # light Red
    $s .= '@echo '."\033[91m".'STorM32 board is using H7, needs to be power cycled !'."\033[0m"."\n";
  }
...
my $BoardCapabilities = 0; # will be set below by ConnectToBoard{)
my $BOARD_CAPABILITY_FOC             =  7; #0x0100
my $BOARD_CAPABILITY_MAVCAMERA       =  5; #0x0400
my $BOARD_CAPABILITY_ONBOARDLOGGING  =  4; #0x0800
...
##    $sa .= " & ".$STorM32LinkStatusText[$index];
  ...
  # NTLogger
  # different handling depending on whether board has onboard logger or not
  $sa = 'NTLogger';
  if( CheckStatus($BoardCapabilities,$BOARD_CAPABILITY_ONBOARDLOGGING) ){
    if( not CheckStatus($status3,$STATUS3_NTLOGGER_PRESENT) ){
      $sa .= ' is not available'; #no SDCard';
    }elsif( CheckStatus($status3,$STATUS3_NTLOGGER_SDCARD_OK) ){
      $sa .= ' is OK';
    }else{
      $sa .= ' is PRESENT (SDCard err)';
    }
  }else{
    if( not CheckStatus($status3,$STATUS3_NTLOGGER_PRESENT) ){
      ##$sa .= ' condition unknown';
      $sa = ''; # don't show it if there is no logger
...
sub DashboardStatusText{
  ...
  $s .= "\r\n\r\n";
  if( $StatusInfoHash{NTLOGGERcondition} ne '' ){
    $s .= $StatusInfoHash{NTLOGGERcondition};
    $s .= "\r\n\r\n";
  }
...
#  }
  ...
  if( $layoutfound == 0 ){
    ...
    my @s = split(' ',$Version);
    TextOut( "\r\n".'Retry with GUI version '.$s[0]."\r\n" );
    DisconnectFromBoard(0);
    return 0;
  }
  #check contrtoller version
  if( $SupportedBGCVersion > 0 and $VerNr < $SupportedBGCVersion ){ #do only do if enabled
    TextOut( "\r\n".'Read... ABORTED!' );
    TextOut( "\r\n".'The firmware version of connected controller is not supported!' );
    my @s = split(' ',$Version);
    TextOut( "\r\n".'Retry with GUI version '.$s[0]."\r\n" );
    DisconnectFromBoard(0);
    return 0;
...
sub GetControllerVersion{
  ...
  return ($version,$name,$board,$layout,$capabilities,$ver);
EXIT:
  return ('','','','','','');
}
...
my @SCRIPT_CMDS= (
    ...
    'DORECENTER_W', 'DORECENTERPITCH_W', 'DORECENTERROLL_W', 'DORECENTERYAW_W',
    'SETRETRACT',
    '' );
...
#        $script_text .= ' '. HexstrToDez( substr($script_hex,$pc+10,2).substr($script_hex,$pc+8,2) );
      ...
      }elsif( $s eq 'DORECENTER_W' ){ # uint8
        $script_text .= $indent.$s;
        $script_text .= ' '. script_U8ToDez($script_hex, $pc+2);
        $pc += 2;
      }elsif( $s eq 'DORECENTERPITCH_W' ){ # uint8
        $script_text .= $indent.$s;
        $script_text .= ' '. script_U8ToDez($script_hex, $pc+2);
        $pc += 2;
      }elsif( $s eq 'DORECENTERROLL_W' ){ # uint8
        $script_text .= $indent.$s;
        $script_text .= ' '. script_U8ToDez($script_hex, $pc+2);
        $pc += 2;
      }elsif( $s eq 'DORECENTERYAW_W' ){ # uint8
        $script_text .= $indent.$s;
        $script_text .= ' '. script_U8ToDez($script_hex, $pc+2);
        $pc += 2;

      }elsif( $s eq 'SETRETRACT' ){ # uint8
        $script_text .= $indent.$s;
        my $v = script_U8ToDez($script_hex, $pc+2);
        $script_text .= ' '. $v;
        $pc += 2;
...
sub script_TextToValue{
  ...
  switch( $Option->{type} ){
    ...
    case 'UINT' {
      if( defined $Option->{multiplier} ) { $text = $text / $Option->{multiplier}; }
...
#TextOut( "!".$code."!\r\n" );
    ...
    switch ( $token ){
      ...
      case 'DORECENTER_W' {
        ...
        ($code,$token) = Script_GetIntValue($code);
        if( $token eq '' ){ $error = 'Valid wait time expected!'; goto ERROR;}
        if( ($token < 0) or ($token > 255) ){ $error = 'Wait time out of range!'; goto ERROR;}
        $hexcode .= UCharToHexstr($token); $pc += 2; #one byte
      }
      case 'DORECENTERPITCH_W' {
        $lastcmd = $token;
        $hexcode .= '32'; $pc += 2;
...
$IniFile->newval( 'FLASH', 'Stm32ProgrammerV1X', $Storm32ProgrammerV1X );
$IniFile->newval( 'FLASH', 'Stm32ProgrammerV3X', $Storm32ProgrammerV3X );
$IniFile->newval( 'FLASH', 'Stm32ProgrammerV56X', $Storm32ProgrammerV56X );
