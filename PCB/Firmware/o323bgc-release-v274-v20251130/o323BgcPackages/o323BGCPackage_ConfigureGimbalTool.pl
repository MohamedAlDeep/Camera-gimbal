...
sub ConfigureGimbalInit
{
  ...
  $ypos += 16;
  $w_ConfigureGimbal->AddCheckbox( -name  => 'configuregimbal_GimbalType_check', -font=> $StdWinFont,
    -pos=> [$xpos,$ypos], -size=> [12,12],
  );
  $w_ConfigureGimbal->AddLabel( -name=> 'configuregimbal_GimbalType_check_label', -font=> $StdWinFont,
    -text=> 'Gimbal Type',
    -pos=> [$xpos+20,$ypos],
    -background=> $ConfigureGimbalBackgroundColor, -foreground=> $CWHITE,
  );
...
Positions',
  ...
  $ypos += 16;
  $w_ConfigureGimbal->AddCheckbox( -name  => 'configuregimbal_FinishFastStartup_check', -font=> $StdWinFont,
    -pos=> [$xpos,$ypos], -size=> [12,12],
  );
  $w_ConfigureGimbal->AddLabel( -name=> 'configuregimbal_FinishFastStartup_check_label', -font=> $StdWinFont,
    -text=> 'Enable Fast Startup',
    -pos=> [$xpos+20,$ypos], -height=> 26,
    -background=> $ConfigureGimbalBackgroundColor, -foreground=> $CWHITE,
  );
  $ypos += 16;
...
Restart Gimbal',
  ...
  # Stuff for the Gimbal Type screen
  ConfigureGimbalInitGimbalTypeScreen($xpos, $ypos);

  # Stuff for the Motor Poles screen
  ConfigureGimbalInitMotorPolesScreen($xpos, $ypos);

  # Stuff for the Yaw Motor Position screen
  ConfigureGimbalInitYawMotorPositionScreen($xpos, $ypos);

  # Ok, Continue, Cancel buttons
  ConfigureGimbalInitOkContinueCancel($xpos, $ypos);
...
{
  $w_ConfigureGimbal->configuregimbal_GimbalType_check->Checked(0);
  $w_ConfigureGimbal->configuregimbal_GimbalType_check->Hide();
...
{
  $w_ConfigureGimbal->configuregimbal_GimbalType_check->Checked(1);
  $w_ConfigureGimbal->configuregimbal_GimbalType_check->Show();
...
# Gimbal Type screen
sub ConfigureGimbalInitGimbalTypeScreen
{
  my $xpos = shift;
  my $ypos = shift;
  $ypos += 21;
  $w_ConfigureGimbal->AddLabel( -name=> 'cg_GimbalType_label', -font=> $StdWinFont,
    -text=> "Gimbal Type", -pos=> [$xpos,$ypos+20],
    -background=> $CGREY128, -foreground=> $CWHITE,
  );
  $w_ConfigureGimbal->AddCombobox( -name=> 'cg_GimbalType', -font=> $StdWinFont,
    -pos=> [$xpos+$w_ConfigureGimbal->cg_GimbalType_label->Width()+3,$ypos+20-3], -size=> [160,160],
    -dropdown=> 1, -vscroll=>1,
  );
  #$w_ConfigureGimbal->cg_GimbalType->SetDroppedWidth(60);
  $w_ConfigureGimbal->cg_GimbalType->Add( ('3-axis','2-axis, pitch roll','2-axis, pitch yaw','1-axis, pitch','1-axis, roll') );

  $w_ConfigureGimbal->cg_GimbalType->Select(0);

#  $w_ConfigureGimbal->AddRadioButton( -name=> 'cg_GimbalType_3axis', -font=> $StdWinFont,
#    -text=> '3-axis', -pos=> [$xpos,$ypos+40], -width=> 160,
#    -background=> $CGREY128, -foreground=> $CWHITE,
#    -checked=>1,
#  );
#  $w_ConfigureGimbal->AddRadioButton( -name=> 'cg_GimbalType_2axis_pitchroll', -font=> $StdWinFont,
#    -text=> '2-axis, pitch roll', -pos=> [$xpos,$ypos+40+1*13], -width=> 160,
#    -background=> $CGREY128, -foreground=> $CWHITE,
#  );
}

sub ConfigureGimbalHideGimbalType
{
  $w_ConfigureGimbal->cg_GimbalType_label->Hide();
  $w_ConfigureGimbal->cg_GimbalType->Hide();
}

sub ConfigureGimbalShowGimbalType
{
  $w_ConfigureGimbal->cg_GimbalType_label->Show();
  $w_ConfigureGimbal->cg_GimbalType->Show();

#  $w_ConfigureGimbal->cg_GimbalType_3axis->Show();
#  $w_ConfigureGimbal->cg_GimbalType_2axis_pitchroll->Show();
}


# Motor Poles screen
sub ConfigureGimbalInitMotorPolesScreen
{
  my $xpos = shift;
  my $ypos = shift;
  $ypos += 21;
...
# Yaw Motor Position/Alignment screen
...
sub ConfigureGimbalInitYawMotorPositionScreen
{
  my $xpos = shift;
  my $ypos = shift;
...
# Ok, Continue, Cancel
# the Continue button also manages the scheduler
sub ConfigureGimbalInitOkContinueCancel
{
  my $xpos= shift;
  my $ypos= shift;
  $xpos= $ConfigureGimbalXsize/2 - 40;
  $ypos= $ConfigureGimbalYsize - 90;
  $w_ConfigureGimbal->AddButton( -name=> 'configuregimbal_Continue', -font=> $StdWinFont,
    -text=> 'Continue', -pos=> [$xpos,$ypos], -width=> 80,
  );
  $w_ConfigureGimbal->AddButton( -name=> 'configuregimbal_OK', -font=> $StdWinFont,
    -text=> 'OK', -pos=> [$xpos,$ypos], -width=> 80,
  );
  $w_ConfigureGimbal->AddButton( -name=> 'configuregimbal_Cancel', -font=> $StdWinFont,
    -text=> 'Cancel', -pos=> [$xpos,$ypos+30], -width=> 80,
  );
}


# this is to block settings while step is running
sub ConfigureGimbalDisableStepICheckboxes
{
  $w_ConfigureGimbal->configuregimbal_GimbalType_check->Disable();
...
# Button Handler, and scheduling
my $ConfigureGimbal_StepNr = 0;
my $ConfigureGimbal_DoStepIIStart = -1;
my $ConfigureGimbal_Imu1No = -1;
my $ConfigureGimbal_Imu2No = -1;

my $ConfigureGimbal_PitchUsed = 0;
my $ConfigureGimbal_RollUsed = 0;
my $ConfigureGimbal_YawUsed = 0;

my $ConfigureGimbal_FastStartupModeUsed = 0;

#my $cMOTORUSAGE_NORMAL = 0;
#my $cMOTORUSAGE_SERVO = 1;
#my $cMOTORUSAGE_STARTPOS = 2;
#my $cMOTORUSAGE_DISABLED = 3;
...
sub configuregimbal_Continue_Click
{
  ...
  if( $w_ConfigureGimbal->configuregimbal_GimbalType_check->Checked() > 0 ){
    ConfigureGimbalDisableStepICheckboxes();
    ConfigureGimbalGimbalType(); return 1;
  }
...
sub ConfigureGimbalShow
{
  ...
  ConfigureGimbalHideGimbalType();
  ConfigureGimbalHideMotorPoles();
  ConfigureGimbalHideAlignYawButtons();

  $w_ConfigureGimbal->configuregimbal_GimbalType_check->Checked(1);
  $w_ConfigureGimbal->configuregimbal_GimbalType_check->Enable();
  ...
  $w_ConfigureGimbal->configuregimbal_FinishFastStartup_check->Checked(0); # do not check as default, will be set below
  $w_ConfigureGimbal->configuregimbal_FinishFastStartup_check->Enable();
  ...
  TextOut( "\r\n".'Read motor usages...' );
  $ConfigureGimbal_PitchUsed = 0;
  $ConfigureGimbal_RollUsed = 0;
  $ConfigureGimbal_YawUsed = 0;
  ...
  $res1 = substr($res1,4,2);
  if ($res1 eq '00') { $ConfigureGimbal_PitchUsed = 1; }
#TextOut( "\r\n".$res1 );
  ...
  $res1 = ExtractPayloadFromRcCmd($res1);
  $res1 = substr($res1,4,2);
  if ($res1 eq '00') { $ConfigureGimbal_RollUsed = 1; }
#TextOut( "\r\n".$res1 );
  ...
  $res1 = ExtractPayloadFromRcCmd($res1);
  $res1 = substr($res1,4,2);
  if ($res1 eq '00') { $ConfigureGimbal_YawUsed = 1; }
#TextOut( "\r\n".$res1 );
  TextOut( ' ok' );
  if (($ConfigureGimbal_PitchUsed != 1) and ($ConfigureGimbal_RollUsed != 1) and ($ConfigureGimbal_YawUsed != 1)) {
    # none enabled, so choose 3-axis
    $ConfigureGimbal_PitchUsed = $ConfigureGimbal_RollUsed = $ConfigureGimbal_YawUsed = 1;
  }
  if (($ConfigureGimbal_PitchUsed == 0) and ($ConfigureGimbal_RollUsed == 0)) {
    $w_ConfigureGimbal->configuregimbal_PitchRollMotorPositions_check->Checked(0)
  }
  if ($ConfigureGimbal_YawUsed == 0) {
    $w_ConfigureGimbal->configuregimbal_YawMotorPosition_check->Checked(0)
  }

  TextOut( "\r\n".'Read further settings...' );
  $ConfigureGimbal_FastStartupModeUsed = 0;
  $Option = $NameToOptionHash{'Startup Mode'};
  $res1 = SendRcCmdwoOut( '03'. UCharToHexstr($Option->{adr}).'00' );
  if( $res1 =~ /[tc]/ ){ goto WERROR; }
  $res1 = ExtractPayloadFromRcCmd($res1);
  $res1 = substr($res1,4,2);
  if ($res1 eq '01') { $ConfigureGimbal_FastStartupModeUsed = 1; }
  $w_ConfigureGimbal->configuregimbal_FinishFastStartup_check->Checked($ConfigureGimbal_FastStartupModeUsed);
#TextOut( "\r\n".$res1 );
  TextOut( ' ok' );
  ...
  _delay_ms(1000); #STorM32 restartsm so wait a bit
  $Option = $NameToOptionHash{'Roll Motor Usage'};
...
#--------------------------------------------------
# ConfigureGimbal: StepI Gimbal Type
#--------------------------------------------------
sub ConfigureGimbalGimbalType
{
  $w_ConfigureGimbal->configuregimbal_Continue->Disable();
  $w_ConfigureGimbal->configuregimbal_Continue->Show();
...
if($ConfigureGimbal_StepNr==0){
    $w_ConfigureGimbal->configuregimbal_Title->Text( 'Gimbal Type' );
    $w_ConfigureGimbal->configuregimbal_WelcomeText1->Text(
      'In this step, the gimbal type will be configured.' );
    $w_ConfigureGimbal->configuregimbal_WelcomeText2->Text(
      'Choose the gimbal type:' . "\r\n". "\r\n". "\r\n". "\r\n". "\r\n". "\r\n".
      "\r\n". "\r\n".
      'Press >Continue< to continue.' );
    ConfigureGimbalShowGimbalType();
    my $ConfigureGimbal_GimbalType;
    if (($ConfigureGimbal_PitchUsed == 1) and
        ($ConfigureGimbal_RollUsed == 1) and
        ($ConfigureGimbal_YawUsed == 1)) {
      $ConfigureGimbal_GimbalType = 0; # '3-axis';
    } elsif (
        ($ConfigureGimbal_PitchUsed == 1) and
        ($ConfigureGimbal_RollUsed == 1) and
        ($ConfigureGimbal_YawUsed != 1)) {
      $ConfigureGimbal_GimbalType = 1; # '2-axis, pitch roll';
    } elsif (
        ($ConfigureGimbal_PitchUsed == 1) and
        ($ConfigureGimbal_RollUsed != 1) and
        ($ConfigureGimbal_YawUsed == 1)) {
      $ConfigureGimbal_GimbalType = 2; # '2-axis, pitch yaw';
    } elsif (
        ($ConfigureGimbal_PitchUsed == 1) and
        ($ConfigureGimbal_RollUsed != 1) and
        ($ConfigureGimbal_YawUsed != 1)) {
      $ConfigureGimbal_GimbalType = 3; # '1-axis, pitch';
    } elsif (
        ($ConfigureGimbal_PitchUsed != 1) and
        ($ConfigureGimbal_RollUsed == 1) and
        ($ConfigureGimbal_YawUsed != 1)) {
      $ConfigureGimbal_GimbalType = 4; # '1-axis, roll';
    } else {
      # SHIT !!!!
      $ConfigureGimbal_GimbalType = 0;
    }
    $w_ConfigureGimbal->cg_GimbalType->Select($ConfigureGimbal_GimbalType);
    $ConfigureGimbal_StepNr = 1;
    $w_ConfigureGimbal->configuregimbal_Continue->Enable();
    $w_ConfigureGimbal->configuregimbal_Continue->Show();
    return;
...
if($ConfigureGimbal_StepNr==1){
    ConfigureGimbalHideGimbalType();
    my $ConfigureGimbal_GimbalType = $w_ConfigureGimbal->cg_GimbalType->SelectedItem();
#TextOut( "\r\n".'GimbalType '.$ConfigureGimbal_GimbalType );
    if ($ConfigureGimbal_GimbalType == 0) { # '3-axis';
      $ConfigureGimbal_PitchUsed = 1;
      $ConfigureGimbal_RollUsed = 1;
      $ConfigureGimbal_YawUsed = 1;
    } elsif ($ConfigureGimbal_GimbalType == 1) { # '2-axis, pitch roll';
      $ConfigureGimbal_PitchUsed = 1;
      $ConfigureGimbal_RollUsed = 1;
      $ConfigureGimbal_YawUsed = 0;
    } elsif ($ConfigureGimbal_GimbalType == 2) { # '2-axis, pitch yaw';
      $ConfigureGimbal_PitchUsed = 1;
      $ConfigureGimbal_RollUsed = 0;
      $ConfigureGimbal_YawUsed = 1;
    } elsif ($ConfigureGimbal_GimbalType == 3) { # '1-axis, pitch';
      $ConfigureGimbal_PitchUsed = 1;
      $ConfigureGimbal_RollUsed = 0;
      $ConfigureGimbal_YawUsed = 0;
    } elsif ($ConfigureGimbal_GimbalType == 4) { # '1-axis, roll';
      $ConfigureGimbal_PitchUsed = 0;
      $ConfigureGimbal_RollUsed = 1;
      $ConfigureGimbal_YawUsed = 0;
    }
#TextOut( "\r\n".$ConfigureGimbal_PitchUsed.' '.$ConfigureGimbal_RollUsed.' '.$ConfigureGimbal_YawUsed );
...
if($ConfigureGimbal_StepNr==0){
    ...
    my $NTBusErrors = $data[$DataBusError_p]; #NT bus error
...
- In a notation like 12N14P the motor pole number is 14.' . "\r\n". "\r\n".
    ...
    ConfigureGimbalShowMotorPoles($ConfigureGimbal_PitchUsed, $ConfigureGimbal_RollUsed, $ConfigureGimbal_YawUsed);
...
if($ConfigureGimbal_StepNr==1){
    ...
    if (($poles >= 8) and ($ConfigureGimbal_PitchUsed == 1)){} else {
      $poles = 14;
    }
    $Option= $NameToOptionHash{'Pitch Motor Poles'};
    $res= SendRcCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. UCharToHexstr($poles).'00' );
    if( $res =~ /[tc]/ ){ goto WERROR; }
    SetOptionField( $Option, $poles );

    $Option= $w_ConfigureGimbal->cg_MotorPolesRoll;
    $poles= $Option->GetString($Option->SelectedItem());
    if (($poles >= 8) and ($ConfigureGimbal_RollUsed == 1)){} else {
      $poles = 14;
    }
    $Option= $NameToOptionHash{'Roll Motor Poles'};
    $res= SendRcCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. UCharToHexstr($poles).'00' );
    ...
    $poles= $Option->GetString($Option->SelectedItem());
    $Option= $NameToOptionHash{'Yaw Motor Poles'};
    if (($poles >= 8) and ($ConfigureGimbal_YawUsed == 1)){} else {
      $poles = 14;
    }
    $res= SendRcCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. UCharToHexstr($poles).'00' );
    if( $res =~ /[tc]/ ){ goto WERROR; }
...
sub ConfigureGimbalMotorDirectionsISetToAuto
{
  $w_ConfigureGimbal->configuregimbal_Continue->Disable();
  $w_ConfigureGimbal->configuregimbal_Continue->Show();
  my $Option; my $res; my $val; my $valstr;
### do the FIRST PART
if($ConfigureGimbal_StepNr==0){
...
if($ConfigureGimbal_StepNr==1){
    ...
    if ($ConfigureGimbal_PitchUsed == 1){ $valstr = '0200'; $val = 2; } else { $valstr = '0000'; $val = 0; }
    $Option= $NameToOptionHash{'Pitch Motor Direction'};
    $res= SendRcCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. $valstr );
    if( $res =~ /[tc]/ ){ goto WERROR; }
    SetOptionField( $Option, $val );

    if ($ConfigureGimbal_RollUsed == 1){ $valstr = '0200'; $val = 2; } else { $valstr = '0000'; $val = 0; }
    $Option= $NameToOptionHash{'Roll Motor Direction'};
    $res= SendRcCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. $valstr );
    if( $res =~ /[tc]/ ){ goto WERROR; }
    SetOptionField( $Option, $val );

    if ($ConfigureGimbal_YawUsed == 1){ $valstr = '0200'; $val = 2; } else { $valstr = '0000'; $val = 0; }
...
#--------------------------------------------------
sub ConfigureGimbalCheckVmaxForAxis
{
    my $axis = shift;
    my $Option= $NameToOptionHash{$axis.' Motor Vmax'};
    my $res= SendRcCmdwoOut( '03'. UCharToHexstr($Option->{adr}).'00' );
    if( $res =~ /[tc]/ ){ return 0; }
    my $vmax = HexstrToDez(substr(ExtractPayloadFromRcCmd($res),4,2));
TextOut( "\r\n".'vmax = !'.$vmax.'!' );
    if ($vmax < 10) {
      $res= SendRcCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. '9600' );
      if( $res =~ /[tc]/ ){ return 0; }
      SetOptionField( $Option, 150 );
    }
    return 1;
}

sub ConfigureGimbalEnableMotorsAndRestart
{
    my $val = 0;
    if ($ConfigureGimbal_PitchUsed == 1) { $val = $val + 1; }
    if ($ConfigureGimbal_RollUsed == 1) { $val = $val + 2; }
    if ($ConfigureGimbal_YawUsed == 1) { $val = $val + 4; }
    my $valstr = '0'.$val;
TextOut( "\r\n".'val = '.$valstr );
    my $s = ExecuteCmdwCrc( 'xV', HexstrToStr($valstr) );
    if( $s eq 'o' ){ TextOut_( 'ok' ); }else{ return 0; }
    #set Motor Usages
    if ($ConfigureGimbal_PitchUsed == 1){
      SetOptionField( $NameToOptionHash{'Pitch Motor Usage'}, 0 );
    } else {
      SetOptionField( $NameToOptionHash{'Pitch Motor Usage'}, 3 );
    }
    if ($ConfigureGimbal_RollUsed == 1){
      SetOptionField( $NameToOptionHash{'Roll Motor Usage'}, 0 );
    } else {
      SetOptionField( $NameToOptionHash{'Roll Motor Usage'}, 3 );
    }
    if ($ConfigureGimbal_YawUsed == 1){
      SetOptionField( $NameToOptionHash{'Yaw Motor Usage'}, 0 );
    } else {
      SetOptionField( $NameToOptionHash{'Yaw Motor Usage'}, 3 );
    }
    return 1;
}
...
sub ConfigureGimbalStepIIStart
{
  $w_ConfigureGimbal->configuregimbal_Continue->Disable();
  $w_ConfigureGimbal->configuregimbal_Continue->Show();
  my $Option; my $res; my $s; my $val; my $valstr;
### do the FIRST PART
if($ConfigureGimbal_StepNr==0){
...
if($ConfigureGimbal_StepNr==1){
    ...
    TextOut_( 'ok' );
    $InfoText .=  'ok'. "\r\n";
    $InfoText .= 'Enable motors and restart the gimbal ... ';
    $w_ConfigureGimbal->configuregimbal_WelcomeText2->Text( $InfoText.'Please wait!' );
    _delay_ms(500);
    ...
    $Option= $NameToOptionHash{'Startup Mode'};
    $res= SendRcCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. '0000' );
    if( $res =~ /[tc]/ ){ goto WERROR; }
    TextOut( ' ok' );
    SetOptionField( $Option, 0 );

    #check Vmax, just in case
    TextOut( "\r\n".'Check Vmax values...' );
    if ($ConfigureGimbal_PitchUsed == 1) {
      if (ConfigureGimbalCheckVmaxForAxis('Pitch') == 0) { goto WERROR; }
    }
    if ($ConfigureGimbal_RollUsed == 1) {
      if (ConfigureGimbalCheckVmaxForAxis('Roll') == 0) { goto WERROR; }
    }
    if ($ConfigureGimbal_YawUsed == 1) {
      if (ConfigureGimbalCheckVmaxForAxis('Yaw') == 0) { goto WERROR; }
    }
    TextOut( ' ok' );
    ...
    TextOut( "\r\n".'Enable motors and restart controller...' );
    if (ConfigureGimbalEnableMotorsAndRestart() == 0) { goto WERROR; }
...
##$ExtendedTimoutFirst = 200; #extend timeout
    ...
    $Option = $NameToOptionHash{'Pitch Motor Direction'};
    if ($ConfigureGimbal_PitchUsed == 1) {
      $res = SendRcCmdwoOut( '03'. UCharToHexstr($Option->{adr}).'00' );
      if( $res =~ /[tc]/ ){ goto WERROR; }
      $res = ExtractPayloadFromRcCmd($res);
      SetOptionField( $Option, substr($res,4,2) );
    } else {
      SetOptionField( $Option, 0 );
    }
    $Option = $NameToOptionHash{'Roll Motor Direction'};
...
$ExtendedTimoutFirst = 200; #extend timeout
      ...
      if ($ConfigureGimbal_PitchUsed == 1) {
        ...
        if( $res =~ /[tc]/ ){ goto WERROR; }
        $res= ExtractPayloadFromRcCmd($res);
        SetOptionField( $Option, HexstrToDez(substr($res,6,2).substr($res,4,2)) );
      } else {
        SetOptionField( $Option, 504 );
      }
...
if($ConfigureGimbal_StepNr == 1){
    ...
    _delay_ms(500);
    #get Yaw Motor Position
    TextOut_( "\r\n".'Get current yaw motor position... ' );
    my $s = ExecuteCmd( 'xy' );
    if( $s eq 'o' ){ TextOut_( 'ok' ); }else{ goto WERROR; }
    #read Yaw Startup Motor Position
...
if( $ConfigureGimbal_StepNr == 0 ){
    ...
    $w_ConfigureGimbal->configuregimbal_WelcomeText2->Text(
      ...
      'into the EEPROM. Also, the motors may still be disabled.' . "\r\n"."\r\n".
...
if( $ConfigureGimbal_StepNr == 1 ){
    $w_ConfigureGimbal->configuregimbal_OK->Disable();
    $w_ConfigureGimbal->configuregimbal_OK->Show();
    my $dofaststartup = $w_ConfigureGimbal->configuregimbal_FinishFastStartup_check->Checked();
    ...
    if( ($dofaststartup < 1) and ($dorestart < 1) and ($dostore < 1) ){ return; } #nothing to do
    my $InfoText = ''; my $s;
    if( $dofaststartup > 0 ){
        TextOut( "\r\n".'Set startup mode to fast... ' );
        $Option= $NameToOptionHash{'Startup Mode'};
        $res= SendRcCmdwoOut( '04'. UCharToHexstr($Option->{adr}).'00'. '0100' );
        if( $res =~ /[tc]/ ){ goto WERROR; }
        TextOut_( 'ok' );
    }
    if( $dorestart > 0 ){
